...
from module_admin.entity.vo.user_vo import (
    ...
    UserProfileModel,
    UserRoleQueryModel,
    UserRoleResponseModel, UserNameResponseModel, UserNameResponsePageQueryModel,
)
...
@userController.get(
    '/list_name', response_model=UserNameResponseModel,
    dependencies=[Depends(CheckUserInterfaceAuth('system:user:list_name'))]
)
async def get_system_user_name_list(
        request: Request,
        query_db: AsyncSession = Depends(get_db),
        user_page_query: UserNameResponsePageQueryModel = Query(),
):
    # 获取分页数据
    user_query_result = await UserService.get_user_name_list_services(query_db, user_page_query, is_page=True)
    logger.info('获取成功')

    return ResponseUtil.success(model_content=user_query_result)

...
account_transactionsController = APIRouter(prefix='/money/account_transactions',
                                           dependencies=[Depends(LoginService.get_current_user)])


@account_transactionsController.get(
    '/sum_transactions', response_model=PageResponseModel,
    dependencies=[Depends(CheckUserInterfaceAuth('money:account_transactions:list'))]
)
async def get_money_latest_balance_sum_service(
        request: Request,
        query_db: AsyncSession = Depends(get_db),
):
    # 获取分页数据
    res = await AccountTransactionsService.get_latest_balance_sum_service(query_db)
    logger.info('获取成功')

    return ResponseUtil.success(data=res)


@account_transactionsController.get('/list',
                                    dependencies=[Depends(CheckUserInterfaceAuth('money:account_transactions:list'))])
async def get_money_account_transactions_list(
        request: Request,
        account_transactions_page_query: AccountTransactionsPageQueryModel = Depends(
            AccountTransactionsPageQueryModel.as_query),
        query_db: AsyncSession = Depends(get_db),
):
    # 获取分页数据
    account_transactions_page_query_result = await AccountTransactionsService.get_account_transactions_list_services(
        query_db, account_transactions_page_query,
        is_page=True)
    logger.info('获取成功')
...
@account_transactionsController.post('',
                                     dependencies=[Depends(CheckUserInterfaceAuth('money:account_transactions:add'))])
...
async def add_money_account_transactions(
    ...
    add_account_transactions_result = await AccountTransactionsService.add_account_transactions_services(query_db,
                                                                                                         add_account_transactions)
...
@account_transactionsController.put('',
                                    dependencies=[Depends(CheckUserInterfaceAuth('money:account_transactions:edit'))])
...
async def edit_money_account_transactions(
    ...
    edit_account_transactions_result = await AccountTransactionsService.edit_account_transactions_services(query_db,
                                                                                                           edit_account_transactions)
...
@account_transactionsController.delete('/{ids}', dependencies=[
    Depends(CheckUserInterfaceAuth('money:account_transactions:remove'))])
...
async def delete_money_account_transactions(request: Request, ids: str, query_db: AsyncSession = Depends(get_db)):
    ...
    delete_account_transactions_result = await AccountTransactionsService.delete_account_transactions_services(query_db,
...
@account_transactionsController.get(
    '/{id}', response_model=AccountTransactionsModel,
    dependencies=[Depends(CheckUserInterfaceAuth('money:account_transactions:query'))]
)
async def query_detail_money_account_transactions(request: Request, id: int, query_db: AsyncSession = Depends(get_db)):
    account_transactions_detail_result = await AccountTransactionsService.account_transactions_detail_services(query_db,
                                                                                                               id)
